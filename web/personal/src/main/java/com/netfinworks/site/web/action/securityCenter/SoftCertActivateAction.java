...
public class SoftCertActivateAction extends BaseAction {
    ...
    public RestResponse checkSoftCert(HttpServletRequest req, OperationEnvironment env) {
		...
		try {
			...
			if("1".equals(securityFlag) || "10".equals(securityFlag)) {//1表示软证书用户,10表示硬证书用户
				CertificationInfoRequest request = new CertificationInfoRequest();
				request.setMemberId(memberId);
				request.setOperatorId(operatorId);
				request.setCertificationType(CertificationType.SOFT_CERTIFICATION.getCode());
	 ...
	 * @return
	 */
	@RequestMapping(value = "/gotoview.htm",method = RequestMethod.POST)
	public ModelAndView gotoview(HttpServletRequest req, OperationEnvironment env) {
		...
		} else if("NotInstallCertUser".equals(btype)) {//是证书用户，但本机为安装证书，跳转安装证书页面
    ...
    public RestResponse idCard(HttpServletRequest req, OperationEnvironment env){
		...
		try {
			...
			}if(idcard.toLowerCase().equals(id_card.toLowerCase())){
...
//				return mv;
//			}
//			if (KaptchaImageAction.validateRandCode(req, req.getParameter("captcha_value")) && idcard.equals(id_card)) {
//				mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/softcert_activate_step2");
//			} else {
//				if(!KaptchaImageAction.validateRandCode(req, req.getParameter("captcha_value"))) {
//					mv.addObject("imageMessage", "验证码错误！");
//				}
...
//    public ModelAndView checkPasswordAndAuthcode(HttpServletRequest req, OperationEnvironment env) throws Exception {
//    	ModelAndView mv = new ModelAndView();
//    	PersonMember user = getUser(req);
//    	String memberId = user.getMemberId();
//    	//获取手机校验码
...
//			pcheck = defaultPayPasswdService.checkPayPwdToSalt(payPasswsReq);
//			if (pcheck == null) {
//				logger.error("软证书激活第三步支付密码校验异常，用户ID[{}]", user.getMemberId());
//				mv.addObject("mobile", mobile);
//				mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/softcert_activate_step2");
//				return mv;
//			}
...
//    }
    ...
    public RestResponse certInstall(HttpServletRequest req, OperationEnvironment env) {
    	...
    	try {
			...
			logger.error("数值软证书安装失败:" + e.getMessage(), e.getCause());
			e.printStackTrace();
		}
    	return result;
    }
