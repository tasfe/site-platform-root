...
@Controller
@RequestMapping("/hardcertUnbind")
public class HardCertUnbindAction extends BaseAction {
	...
	public ModelAndView UnbindCertStep1(HttpServletRequest req, OperationEnvironment env) {
		...
		mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/hardcert_unbind_step1");
		return mv;
	}
	...
	public ModelAndView UnbindCertStep2(HttpServletRequest req, OperationEnvironment env) throws Exception {
		...
		String mobile = user.getMobile();
		String memberId = user.getMemberId();
		String enterpriseName = user.getLoginName();
		mv.addObject("mobile", mobile);
		...
		try {
			...
			if (pcheck.isSuccess() && busCode.toLowerCase().equals(id_card.toLowerCase())) {
				mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/hardcert_unbind_step2");
			} else {
				if (!pcheck.isSuccess()) {
	...
	public ModelAndView certUnbind(HttpServletRequest req, OperationEnvironment env) throws BizException {
	    ...
	    if(result.isSuccess()) {
	    	mv.addObject("certSn", certSn);
	    	mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/hardcert_unbind_step3");
	    } else {
	    	mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/hardcert_unbind_step2");
	    }
		return mv;
