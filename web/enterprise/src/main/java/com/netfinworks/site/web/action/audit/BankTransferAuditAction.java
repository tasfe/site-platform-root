 ...
 */
@Controller
public class BankTransferAuditAction extends BaseAction {
	 ...
	 */
	@RequestMapping(value = "/my/transferBankAudit.htm", method = {
	...
	public ModelAndView bankTransferAudit(HttpServletRequest request,
		...
		Map<String, Object> auditData = JSONObject.parseObject(audit
				.getAuditData());
		if(audit.getAuditType().equals(AuditType.AUDIT_PAY_TO_CARD.getCode())||audit.getAuditType().equals(AuditType.AUDIT_TRANSFER.getCode())){
			String operatorId = (String)auditData.get("operatorId");
			try{
				OperatorVO operator = operatorService.getOperatorById(operatorId, env);
				data.put("operatorName", operator.getLoginName());
			}catch(Exception e){
		...
		if (AuditSubType.BATCH.getCode().equals(subType)) {// 批量查看详情
			...
			req.setBatchNo(audit.getTranVoucherNo());
...
//		boolean isDraw = auth(request, FunctionType.EW_MY_APPROVE.getCode());
	 ...
	 * @throws ServiceException
	 */
	@RequestMapping(value = "/my/transferBankAuditConfirm.htm", method = {
			RequestMethod.POST, RequestMethod.GET })
	public @ResponseBody RestResponse transferAuditConfirm(HttpServletRequest request,HttpSession session,OperationEnvironment env,
