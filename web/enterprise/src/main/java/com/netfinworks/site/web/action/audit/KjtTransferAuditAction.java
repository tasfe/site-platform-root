 ...
 */
@Controller
public class KjtTransferAuditAction extends BaseAction {
	 ...
	 */
	@RequestMapping(value = "/my/transferKjtAudit.htm", method = {
			RequestMethod.POST, RequestMethod.GET })
	public ModelAndView transferKjtAudit(HttpServletRequest request,
		...
		if (AuditSubType.BATCH.getCode().equals(subType)) {// 批量查看详情
			...
			data.put("memo", transferInfo);//设置付款理由
...
//		boolean isDraw = auth(request, FunctionType.EW_MY_APPROVE.getCode());
	 ...
	 * @throws ServiceException
	 */
	@RequestMapping(value = "/my/transferKjtAuditConfirm.htm", method = {
			RequestMethod.POST, RequestMethod.GET })
	public @ResponseBody RestResponse transferKjtAuditConfirm(HttpServletRequest request,HttpSession session,OperationEnvironment env,
			...
			logger.info(LogUtil.generateMsg(OperateTypeEnum.TRANSFER_EXAMINE, user, env,
                ("reject".equals(operate) ? "拒绝" : "通过") + "[" + operate + "]"));
			...
			if(AuditSubType.SINGLE.getCode().equals(subType)){
	            ...
	            try {
					...
					restP.setSuccess(response.isSuccess());
