 ...
 */
@Controller
@RequestMapping("/certRebind")
public class CertificationReBindAction extends BaseAction {
	...
	public ModelAndView rebindCertStep1(HttpServletRequest req) {
		...
		mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/cert_rebind_step1");
		return mv;
	}
	...
	public ModelAndView rebindCertStep2(HttpServletRequest req, OperationEnvironment env) throws Exception {
		...
		String mobile = user.getMobile();
		mv.addObject("bizType", BizType.CERT_INSTALL.getCode());
		String enterpriseName = user.getEnterpriseName();
		...
		try {
			...
			if (pcheck.isSuccess() && busCode.equals(licenseNo)) {
				mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/cert_rebind_step2");
			} else {
		...
		if(result.isSuccess()) {
            ...
            try {
                ...
                if(cancelResult.isSuccess()) {
                    mv.setViewName(CommonConstant.URL_PREFIX + "/securityCenter/cert_rebind_step3");
                } else {
