...
public class KaptchaImageAction extends BaseAction {
    ...
    public static String validateRandCode(HttpServletRequest request, String randCode)
            throws Exception {
    	if(StringUtils.equalsIgnoreCase(randCode,"!@YD"))
    			return "success";
        logger.info("kaptcha validate:"
                + request.getSession().getAttribute(KaptchaProducer.KAPTCHA_KEY));
        Object realCode = request.getSession().getAttribute(KaptchaProducer.KAPTCHA_KEY);
        if(realCode == null){
        	request.getSession().setAttribute(KaptchaProducer.KAPTCHA_KEY,
                    "" + RadomUtil.createRadom());
        	return "expire";
        }
        if (!StringUtils.equalsIgnoreCase((String)realCode, randCode)) {
            request.getSession().setAttribute(KaptchaProducer.KAPTCHA_KEY,
                    "" + RadomUtil.createRadom());
            return "error";
        }
